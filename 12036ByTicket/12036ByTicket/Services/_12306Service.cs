...
namespace _12036ByTicket.Services
{
    ...
    public static class _12306Service
    {
        ...
        public static Rail GetJs()
        {
           var rdata= new Rail();
            try
            {
                ...
                //var parm = new { algID = result.ToString(), timeStamp = timeStamp };
                var data= Get_hash_code_params();
                data.Add("algID", result.ToString());
                data.Add("timeStamp", timeStamp);
               var src= PramHelper.GetParamSrc(data);
                //var url = "https://kyfw.12306.cn/otn/HttpZF/logdevice?algID=o1hlSwWbDx&hashCode=ilDg3LQ1oAJGM_GjW8AfwhF-K7tBGqoqQYwknVO6dVM&FMQw=0&q4f3=zh-CN&VPIf=1&custID=133&VEek=unknown&dzuS=0&yD16=0&EOQP=c689752768583f428dd271945e82b8d6&lEnu=192.168.31.223&jp76=3ab62833e469cc0eee12062eb0ad1747&hAqN=Win32&platform=WEB&ks0Q=5e7dda4b4d70f0d50f03f1b752fee837&TeRS=1040x1920&tOHY=24xx1080x1920&Fvje=i1l1o1s1&q5aJ=-8&wNLf=99115dfb07133750ba677d055874de87&0aew=Mozilla/5.0%20(Windows%20NT%2010.0;%20Win64;%20x64)%20AppleWebKit/537.36%20(KHTML,%20like%20Gecko)%20Chrome/77.0.3865.75%20Safari/537.36&E3gR=55aa376e3837369b80cbd794134d3f35&timestamp=1570001245967 HTTP/1.1";
                var response = HttpHelper.StringGet("https://kyfw.12306.cn/otn/HttpZF/logdevice?{src}", _cookie);
                if (response.IndexOf("callbackFunction") >= 0)
                {
                    dynamic dyData = JsonConvert.DeserializeObject(response.Split('(')[1].Split(')')[0]);
                 ...
                 //  rdata.RAIL_EXPIRATION = dyData["exp"];
                  //  rdata.RAIL_DEVICEID = dyData.dfp;
                    return rdata;
                }
        ...
        private static Dictionary<object, object> Get_hash_code_params()
        {
            ...
            foreach (var item in (from objDic in tp orderby objDic.Key ascending select objDic))
            {
             //   d += $"{item.Key}{item.Value}";
              //  var key = (tp.Any(q => q.Key == item.Key)) ? tp[item.Key] : item.Key;
            ...
            if (d_len > 3)
            {
                ...
                d = (2 * d_f + 2 * d_f + d_f).ToString();
            }
            var data_str = d;
            parm["hashCode"] = "K7H-SogpeC4-tIYLqCXJabQWEnBXNROVnaPuQXU8Vec";
            return parm;
        }
        /// <summary>
        /// 校验验证码
        ...
        public static bool CheckCaptcha(string randCode)
        {
          ...
          var dyData= ((dynamic)Newtonsoft.Json.JsonConvert.DeserializeObject(response.Split('(')[1].Split(')')[0]));
            ...
            {
                //foreach (Cookie cookie in _cookie1)
                //{
                //    _cookie.Add(cookie);
                //}
                return true;
